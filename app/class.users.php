...
class users implements iUsers
{
	...
	{
		global $engine;
		if($engine->num_rows("SELECT * FROM users WHERE ip_reg = '" . $_SERVER['REMOTE_ADDR'] . "'") == 15)
		{
			return true;
		}
								...
								if($template->form->reg_password == $template->form->reg_rep_password)
								{
									...
									if($this->isBanned($_SERVER['REMOTE_ADDR']) == false)
									{
										if(!$this->hasClones($_SERVER['REMOTE_ADDR']))
										{
											...
											header('Location: ' . $_CONFIG['hotel']['url'] . '/index.php?url=me');
											exit;
										}
										else
										{
											$template->form->error = 'Sorry, maar je kan maximum met 10 accounts per ip.';
										}
									}
									else
									{
	...
	final public function updateAccount()
	{
		...
		if(isset($_POST['account']))
		{
			...
			if(isset($_POST['acc_new_descr']) && $_POST['acc_new_descr'] != $this->getInfo($_SESSION['user']['id'], 'description'))
			{
				if ($this->getInfo($_SESSION['user']['id'], 'rank') >= 5)
				{
					$this->updateUser($_SESSION['user']['id'], 'description', $engine->secure($_POST['acc_new_descr']));
					header('Location: '.$_CONFIG['hotel']['url'].'/index.php?url=account');
					exit;
				}
			}
			
			if(isset($_POST['acc_username']) && $_POST['acc_username'] != $this->getInfo($_SESSION['user']['id'], 'username'))
			{
				if($this->validEmail($_POST['acc_email']))
				{
					$this->updateUser($_SESSION['user']['id'], 'username', $engine->secure($_POST['acc_username']));
					header('Location: '.$_CONFIG['hotel']['url'].'/account');
					exit;
				}
				else
			...
			if(isset($_POST['acc_thuis']) && $_POST['acc_thuis'] != $this->getInfo($_SESSION['user']['id'], 'land'))
			{
				if($this->validEmail($_POST['acc_email']))
				{
					$this->updateUser($_SESSION['user']['id'], 'land', $engine->secure($_POST['acc_thuis']));
					header('Location: '.$_CONFIG['hotel']['url'].'/account');
					exit;
				}
				else
				{
					$template->form->error = 'Geen foefelare met mijn form!';
					return;
				}
			}
			if(isset($_POST['acc_thuis2']) && $_POST['acc_thuis2'] != $this->getInfo($_SESSION['user']['id'], 'land2'))
			{
				if($this->validEmail($_POST['acc_email']))
				{
					$this->updateUser($_SESSION['user']['id'], 'land2', $engine->secure($_POST['acc_thuis2']));
					header('Location: '.$_CONFIG['hotel']['url'].'/account');
					exit;
				}
				else
